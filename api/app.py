...
app = FastAPI(title='Credit Risk Prediction API', description='ML-powered loan default risk assessment with drift monitoring', version='1.0.0')
...
class LoanApplication(BaseModel):
    ...
    class Config:
        json_schema_extra = {'example': {'person_age': 30, 'person_income': 60000, 'person_home_ownership': 'MORTGAGE', 'person_emp_length': 5.0, 'loan_intent': 'PERSONAL', 'loan_grade': 'B', 'loan_amnt': 10000, 'loan_int_rate': 11.5, 'loan_percent_income': 0.17, 'cb_person_default_on_file': 'N', 'cb_person_cred_hist_length': 8}}
...
async def predict(application: LoanApplication):
    ...
    return PredictionResponse(prediction=prediction, default_probability=round(probability, 4), risk_level=risk_level, confidence=round(max(probability, 1 - probability), 4), timestamp=datetime.now().isoformat())
...
async def predict_batch(request: BatchRequest):
    ...
    for app_data in request.applications:
        ...
        results.append({'default_probability': round(prob, 4), 'prediction': 'DEFAULT' if prob >= 0.5 else 'FULLY_PAID'})
