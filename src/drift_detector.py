"""
Drift Detection Module for Credit Risk Prediction.

Monitors data drift between reference (training) data and new production data.
Uses statistical tests to detect when incoming data distributions shift
significantly from what the model was trained on.

Methods:
- Population Stability Index (PSI)
- Kolmogorov-Smirnov Test
- Categorical drift via distribution comparison

Since this dataset doesn't have timestamps, we simulate drift by:
1. Modifying feature distributions to mimic an economic downturn
2. Comparing reference vs production data
"""
...
class DriftDetector:
    """
    Detects data drift between reference (training) data and new production data.

    PSI Interpretation:
        PSI < 0.1  -> No significant drift (model is fine)
    ...
    def __init__(self, reference_data):
        """
        Initialize with reference (training period) data.

        Args:
            reference_data: DataFrame of data the model was trained on
        """
    ...
    def calculate_psi(self, reference, current, bins=10):
        ...
        Higher PSI = more drift between the two distributions.
        """
        ...
        # Convert to proportions (epsilon avoids log(0))
        eps = 1e-6
    ...
    def ks_test(self, reference, current):
        ...
        Returns:
            statistic: how different the distributions are (0 = identical)
            p_value: probability they're from the same distribution
                     (low p = different distributions)
        """
        ref_clean = reference.dropna()
        curr_clean = current.dropna()
    ...
    def categorical_drift(self, reference, current):
        """Check drift in categorical features using distribution comparison."""
        ...
        return {
            ...
            'new_categories': list(set(curr_dist.index) - set(ref_dist.index)),
            'missing_categories': list(set(ref_dist.index) - set(curr_dist.index)),
            'drifted': max_diff > 0.1
        }
    ...
    def detect_drift(self, current_data):
        ...
        Args:
            current_data: DataFrame of new/production data

        Returns:
            dict: Complete drift report with per-feature analysis
        """
        report = {
        ...
        for col in self.numerical_cols:
            ...
            report['features'][col] = {
                ...
                'curr_mean': round(float(curr_col.mean()), 2),
                'mean_shift': round(float(curr_col.mean() - ref_col.mean()), 2),
                'ref_std': round(float(ref_col.std()), 2),
                'curr_std': round(float(curr_col.std()), 2),
                'status': status
            }
        ...
        for col in self.categorical_cols:
            ...
            cat_result = self.categorical_drift(
            ...
            report['features'][col] = {
                'type': 'categorical',
                'max_category_diff': round(cat_result['max_category_diff'], 4),
                'new_categories': cat_result['new_categories'],
                'missing_categories': cat_result['missing_categories'],
                'status': status
            }
        ...
        report['overall']['drift_percentage'] = round(
            n_drifted / max(total, 1) * 100, 1
        )
        ...
        if n_drifted >= total * 0.3:
            ...
            report['overall']['recommendation'] = (
                'RETRAIN MODEL IMMEDIATELY - significant drift in 30%+ features'
            )
        elif n_drifted > 0 or n_warning >= total * 0.3:
            report['overall']['severity'] = 'warning'
            report['overall']['drift_detected'] = True
            report['overall']['recommendation'] = (
                'INVESTIGATE - some features showing drift, monitor closely'
            )
        else:
    ...
    def print_report(self, report):
        """Pretty print the drift detection report."""
        print("\n" + "=" * 65)
        ...
        severity_label = {
            'none': 'STABLE',
            'warning': '!! WARNING !!',
            'critical': '!!! CRITICAL - RETRAIN NEEDED !!!'
        }
...
def simulate_economic_downturn(df, severity=1.0):
    """
    Simulate an economic downturn by modifying the data distributions.

    This mimics what would happen in production when economic conditions change:
    - Incomes drop
    - People request larger loans
    - Interest rates increase
    - More people default on existing credit
    - More renters (people lose homes)

    Args:
        df: Original reference data
        severity: 0.5 = mild downturn, 1.0 = moderate, 2.0 = severe
    """
    drifted = df.copy()
    n = len(drifted)
    ...
    # Loan-to-income ratio worsens
    drifted['loan_percent_income'] = (
        drifted['loan_amnt'] / drifted['person_income']
    ).clip(0, 0.83).round(2)

    # More defaults on file
    flip_mask = np.random.random(n) < (0.15 * severity)
    drifted.loc[flip_mask, 'cb_person_default_on_file'] = 'Y'

    # More people renting (lost homes)
    ...
    # Loan grades shift toward riskier
...
def run_drift_detection():
    """Run drift detection with the real dataset."""
    ...
    # ============================================
    # TEST 3: Severe economic downturn
    # ============================================
    print("\n" + "=" * 65)
